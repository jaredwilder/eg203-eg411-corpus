...
Core definitions and closure bridges.
No analytic NT in this file.
-/
...
def PrimeCountInBox (m D : Nat) : Nat :=
  ((TriBox D).filter (fun x => Nat.Prime (V m x.1 x.2))).card
...
theorem positive_count_gives_witness
  ...
  have hnonempty : (((TriBox D).filter (fun x => Nat.Prime (V m x.1 x.2))).Nonempty) := by
