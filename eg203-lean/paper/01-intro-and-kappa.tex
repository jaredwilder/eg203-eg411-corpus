...
record (the largest known witness diagonal for $m \leq 10^{10}$ is
$D = 26$ (sweep complete to $m \leq 10^{10}$), see \S\ref{sec:empirical}). The reader interested only in the
...
\begin{enumerate}
  ...
  \item In the $\kappa = 0$ regime the parity barrier is absent, so the
    ...
    \cite{Hooley1972}) suffices for a lower bound. No Type-II bilinear
...
below as Remark~\ref{rem:EP-backup} using only the unconditional
...
The dyadic decomposition forces the key dependence on the
\emph{quantitative} rank-$2$ Pappalardi bound
\eqref{eq:papp-refined}. We have stated this bound from
\cite{PappalardiSusa2010} and used the exponent $c_{0} \log z / j$, but
the verbatim form in their paper has not been line-checked. A
self-contained fallback using only the unconditional
Erd\H{o}s--Pomerance bound (Remark~\ref{rem:EP-backup}) gives the
weaker conclusion $F(z) = O(\sqrt{z} \log z)$, which is NOT enough for
$\kappa_{V} = 0$. So the $\kappa = 0$ statement as currently written
is \emph{conditional on the Pappalardi--Susa quantitative bound in the
form \eqref{eq:papp-refined}}; an unconditional $\kappa = 0$ statement
requires either (a) verification of \cite[Thm 2]{PappalardiSusa2010}
verbatim, or (b) a direct rank-$2$ adaptation of Pomerance 1985 with
explicit log-power decay.

We flag this in \S\ref{sec:limitations}.
